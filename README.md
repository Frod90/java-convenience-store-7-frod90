...
+ [x] 프로 모션 증정 수량을 계산할 수 있다
+ [x] 프로 모션이 적용되지 않는 수량을 계산할 수 있다
+ [x] 무료로 얻을 수 있는 프로 모션 추가 수량을 계산할 수 있다
...
+ [x] 제품을 조회할 수 있다.
    + [x] 없는 제품을 조회할 수 없다.
    + [x] 재고가 부족한 상품은 조회할 수 없다.
+ [x] 재고 수량을 차감할 수 있다.
    + [x] 재고를 초과하여 차감할 수 없다.
+ [x] 프로모션 결과를 계산할 수 있다.
    + [x] 프로모션이 활성화되어 있지 않으면 프로모션을 계산할 수 없다.
+ [x] 프로모션 적용이 가능한지 확인할 수 있다.
+ [x] 구매 수량이 총 재고 수량을 초과할 수 없다.
+ [x] 프로모션 결과를 계산할 수 있다.
+ [x] 프로모션이 활성화 되어 있지 않으면 기본 프로모션 결과를 얻는다.
...
+ [x] 프로모션 결과를 생성할 수 있다.
+ [x] 추가 무료 프로모션 수량을 가지고 있는지 확인할 수 있다.
+ [x] 적용 불가능한 프로모션 수량을 가지고 있는지 확인할 수 있다.

### 멤버십

+ [x] 멤버십을 생성할 수 있다.
+ [x] 멤버십을 할인율을 조회할 수 있다

### 구매 결과

+ [x] 구매 결과를 생성할 수 있다.
+ [x] 총 구매 금액를 계산할 수 있다
+ [x] 프로모션 할인 금액을 계산할 수 있다.
+ [x] 프로모션 혜택을 받지 않은 상품의 구매 금액을 얻을 수 있다.
+ [x] 구매 결과 목록을 생성할 수 있다.
+ [x] 총 구매 수량을 계산할 수 있다.
+ [x] 총 구매 금액을 계산할 수 있다.
+ [x] 프로모션 할인 총 금액을 계산할 수 있다.
+ [x] 멤버십 할인 금액을 계산할 수 있다.
+ [x] 총 결제 금액을 계산할 수 있다.

### 사용자 입력

+ [x] 주문을 입력 받을 수 있다.
    + [x] 주문 입력시 공백은 허용하지 않는다
    + [x] prefix, suffix 사용해서 주문을 입력해야한다.
    + [x] 제품 이름과 수량 사이는 구분자로 구별해야 한다.
    + [x] 지정된 위치에 수량을 입력해야한다.
+ [x] 대소문자 구분 없이 Y 또는 N을 입력하여 의사결정을 입력할 수 있다.
    + [x] 의사 결정 입력시 Y/N을 제외한 입력은 허용하지 않는다.